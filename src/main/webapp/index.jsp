...
<script type="text/javascript">
	...
	function addtocart(id){
		...
		$.ajax({
            type: 'get',
            async: true,
            url: 'AddToCart?productId=' + id + '&size=' + size,
            success: function(data) {
            	if(data == 'ok'){
	...
	function productDisplayFunction(productId){
		$('#productdisplaymodal').on('show.bs.modal', function() {
			...
			$.ajax({
	            ...
	            type: 'get',
	            async: true,
	            url: 'GetProductInfo?productId=' + productId,
	            success: function(data) {
	...
	function showProductHints(productName){
		if(productName!=""){
    		$.ajax({
    		    url : "ShowSuggestions?term=" + productName,
    		    type : "GET",
    		    async : true,
    		    success : function(data) {
    		    	if(data != ""){
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <input type="submit" class="btn btn-info" value="Login"> <a href = "adminLoginSignup.jsp"><input type="button" class="btn btn-info" value="Login as Admin"></a>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
	      ...
	      if(session.getAttribute("sessionEmail") == null){
			  ...
			  Connection con = DriverManager.getConnection("jdbc:mysql://localhost/amazon?user=root&password=");
...
<nav class="navbar navbar-expand-lg navbar-light sticky-top" style="background-color:#32127A;">
  <a class="navbar-brand" href="#"><h2 style="color:white;">RedStore</h2></a>
    ...
    <ul class="navbar-nav">
      <li class="nav-item active">
      	...
      	if(session.getAttribute("sessionEmail") == null){
      		...
      		%>
      		<button onclick="logoutbutton()" class="btn btn-md" id="navbarprofilename"><i class="fa fa-power-off"></i>&nbspLogout</button>
      		<%
      	}
      	%>
...
<br>
	...
	<%
	Class.forName("com.mysql.jdbc.Driver");
	Connection  conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/amazon","root",null);	
	PreparedStatement statement1 = conn.prepareStatement("select * from products where productCategory='Topwear' order by productId desc limit 4");
	ResultSet rs1 = statement1.executeQuery();
	while(rs1.next()){
		%>
		<div class="col-xl-3">
		<%String productImagePath = "images\\" + rs1.getString(1) + "\\productImage.jpg?" + Math.random(); %>
...
<br>
	...
	<%
	Class.forName("com.mysql.jdbc.Driver");
	Connection  connn=DriverManager.getConnection("jdbc:mysql://localhost:3306/amazon","root",null);	
	PreparedStatement statement2 = connn.prepareStatement("select * from products where productCategory='Bottomwear' order by productId desc limit 4");
	ResultSet rs2 = statement2.executeQuery();
	while(rs2.next()){
		%>
		<div class="col-xl-3">
		<%String productImagePath = "images\\" + rs2.getString(1) + "\\productImage.jpg?" + Math.random(); %>
